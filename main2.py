# -*- coding: utf-8 -*-
# このファイルはノーズ電装が取得したバイナリコードを自動的にCSVファイルに変換し，解析を行うコードですが，使用しないでください．再起動データをエラーとして扱います．
...
while True:
  ...
  if len(binary_data_d) == 0 or len(binary_data_t) == 0 or len(binary_data_p) == 0:
    break
  ...
  if int_data_d == 4294967295 and int_data_t == 4294967295 and int_data_p == 4294967295:
    print('ERROR: Double Data')
    select = input()

  List_int_data_d.append(int_data_d)
  List_int_data_t.append(int_data_t)
  List_int_data_p.append(int_data_p)

import numpy as np

sensorValue = np.array(List_int_data_d)
Temperature = np.array(List_int_data_t)/100
Pressure = np.array(List_int_data_p)/10000

for i in range(SAMPLING_RATE*10):
  average_cac_list.append(List_int_data_d[i])

sensorOffset = sum(average_cac_list)/len(average_cac_list)
sensorValue_C = sensorValue - sensorOffset

voltage = sensorValue_C * (voltageMax / sensorMax)
